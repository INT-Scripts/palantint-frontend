...
export default function AdminDashboardPage() {
    ...
    useEffect(() => {
        fetchPrivate("/users/me").then(data => setMe(data)).catch(() => { });
        fetchPrivate("/admin/telemetry").then(data => setTelemetry(data)).catch(() => { });
        fetchPrivate("/relationship-types").then(data => setRelTypes(data || [])).catch(() => { });
        loadGrid("");
    }, []);

    const loadGrid = (q: string) => {
        setLoading(true);
        fetchPrivate(`/admin/students/grid?limit=200&search=${encodeURIComponent(q)}`)
            .then(data => {
                setStudents(data || []);
                setLoading(false);
            })
    ...
    const handleCellSave = async (id: string, field: string, value: string) => {
        ...
        try {
            await fetchPrivate(`/admin/students/${id}`, {
                method: "PATCH",
    ...
    const handleProvisionUser = async () => {
        ...
        setProvLoading(true);
        try {
            const res = await fetchPrivate("/admin/users/provision", {
                method: "POST",
            ...
            fetchPrivate("/admin/telemetry").then(data => setTelemetry(data));
        } finally {
    ...
    const revokeUser = async (student: any) => {
        ...
        try {
            await fetchPrivate(`/admin/users/${student.user_id}`, { method: "DELETE" });
    ...
    const toggleAdmin = async (student: any) => {
        try {
            await fetchPrivate(`/admin/users/${student.user_id}`, {
                method: "PATCH",
    ...
    const deleteRelType = async (id: string) => {
        ...
        try {
            await fetchPrivate(`/admin/relationship-types/${id}`, { method: "DELETE" });
    ...
    const handleCreateRelType = async (e: React.FormEvent) => {
        ...
        setCreatingRel(true);
        try {
            const newType = await fetchPrivate("/admin/relationship-types", {
                method: "POST",
