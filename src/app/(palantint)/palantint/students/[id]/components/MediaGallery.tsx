...
import { fetchPrivate, API_BASE_URL } from "@/lib/api";
...
export default function MediaGallery({ studentId, initialMedia, themeColor }: MediaGalleryProps) {
    ...
    const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const newMedia = await fetchPrivate(`/students/${studentId}/media`, {
                method: "POST",
                body: formData,
            });
    ...
    const handleDelete = async (mediaId: string, e?: React.MouseEvent) => {
        ...
        try {
            await fetchPrivate(`/media/${mediaId}`, { method: "DELETE" });
                ...
                {media.length === 0 ? (
                    ...
                    media.map((item) => (
                        ...
                        >
                            ...
                            {item.type === "IMAGE" ? (
                                <img
                                    src={`${API_BASE_URL}/private/media/${item.id}/file${token ? `?token=${token}` : ""}`}
                                    alt="Student upload"
                ...
                >
                    ...
                    >
                        {selectedMedia.type === "IMAGE" ? (
                            <img
                                src={`${API_BASE_URL}/private/media/${selectedMedia.id}/file${token ? `?token=${token}` : ""}`}
                                alt="Enlarged media"
                        ...
                        <div className="absolute bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-96 bg-zinc-950/90 border border-zinc-800 p-4 backdrop-blur-md shadow-2xl flex flex-col gap-4">
                            ...
                            <div className="flex gap-2">
                                <a
                                    href={`${API_BASE_URL}/private/media/${selectedMedia.id}/file${token ? `?token=${token}` : ""}`}
