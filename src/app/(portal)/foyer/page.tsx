...
import { Layers, FileText, Users, MapPin, X } from "lucide-react";
...
export default function PublicFoyerPage() {
  ...
  return (
    <section className="flex-1 max-w-[1600px] w-full mx-auto px-4 sm:px-6 py-8 sm:py-12 relative z-10 space-y-8">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {Object.keys(BUILDINGS).map(b => (
                  <button key={b} onClick={() => { setActiveBuilding(b); if (!BUILDINGS[b].find(f => f.value === activeFloor)) setActiveFloor(BUILDINGS[b][0].value); }}
                    className={`px-3.5 py-1.5 text-xs font-bold font-mono rounded-xl transition-all cursor-pointer ${activeBuilding === b ? "bg-rose-600 text-white shadow-sm" : "bg-stone-100 dark:bg-stone-800/60 text-zinc-700 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-800"}`}>
                    {b}
                  </button>
                ))}
              </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {(BUILDINGS[activeBuilding] || []).map(f => (
                  <button key={f.value} onClick={() => { setActiveFloor(f.value); setSelectedRoomId(null); }}
                    className={`px-3.5 py-1.5 text-xs font-bold font-mono rounded-xl transition-all cursor-pointer ${activeFloor === f.value ? "bg-zinc-950 dark:bg-stone-100 text-white dark:text-zinc-950 shadow-sm" : "bg-stone-100 dark:bg-stone-800/60 text-zinc-600 dark:text-stone-400 hover:bg-stone-200 dark:hover:bg-stone-800"}`}>
                    {f.label}
                  </button>
                ))}
              </div>
      ...
      {/* Two-column: SVG Map + Room Directory */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 w-full space-y-6">
          ...
          {selectedRoomId && foyerMap[selectedRoomId] && (() => {
            ...
            return (
              <div className="bg-white dark:bg-stone-900 border-2 border-rose-500/80 rounded-3xl p-5 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-zinc-200/80 dark:border-stone-800 pb-3">
                  <div className="flex items-center gap-2.5">
                    <span className="w-3 h-3 rounded-full bg-rose-500 animate-pulse" />
                    <h3 className="text-lg font-extrabold font-mono text-zinc-950 dark:text-stone-50">Local {selectedRoomId}</h3>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-1 text-xs font-bold font-mono rounded-lg bg-rose-600 text-white">{detail.type || "Club"}</span>
                    <button onClick={() => setSelectedRoomId(null)} className="p-1 rounded-lg text-zinc-400 hover:text-zinc-800 dark:hover:text-stone-200 hover:bg-stone-100 dark:hover:bg-stone-800 transition-all cursor-pointer">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                </div>
                <div className="bg-stone-50 dark:bg-stone-950 p-3.5 rounded-2xl border border-zinc-200/80 dark:border-stone-800 font-mono">
                  ...
                  <span className="font-extrabold text-rose-600 dark:text-rose-400 text-base block">{detail.club_name || detail.raw_name || "Non attribué"}</span>
                  {detail.association_of_origin && <span className="text-xs text-zinc-500 dark:text-stone-400 mt-1 block">Tutelle: {detail.association_of_origin}</span>}
                </div>
              </div>
            );
          })()}
          ...
          <div className="bg-white dark:bg-stone-900 border-2 border-zinc-200/90 dark:border-stone-800 rounded-3xl overflow-hidden shadow-sm min-h-[450px] flex flex-col">
            <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-200/80 dark:border-stone-800 bg-stone-50/80 dark:bg-stone-950/60 shrink-0">
              ...
              <span className="text-[10px] font-mono text-zinc-500 dark:text-stone-500 uppercase font-bold">{currentFloorRooms.length} Locaux</span>
            </div>
              ...
              {loading ? (
                ...
                <div className="max-h-[450px] overflow-y-auto pr-1 space-y-2.5 custom-scrollbar">
                  {currentFloorRooms.map(room => {
                    ...
                    return (
                      <div key={room.room_id} onClick={() => setSelectedRoomId(room.room_id)}
                        className={`p-3.5 rounded-2xl border transition-all cursor-pointer font-mono flex items-center justify-between ${isSelected ? "bg-rose-500/10 border-rose-500 text-rose-600 dark:text-rose-400 shadow-xs" : "bg-stone-50/70 dark:bg-stone-950/50 border-zinc-200/80 dark:border-stone-800/80 hover:bg-stone-100 dark:hover:bg-stone-800 text-zinc-800 dark:text-stone-200"}`}>
                        ...
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-stone-200/70 dark:bg-stone-800 text-zinc-600 dark:text-stone-300 uppercase shrink-0">{room.type || "Club"}</span>
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="bg-white dark:bg-stone-900 border-2 border-zinc-200/90 dark:border-stone-800 rounded-3xl p-5 shadow-sm h-[500px] flex flex-col relative">
          ...
          <div className="flex-1 rounded-2xl overflow-hidden bg-stone-50 dark:bg-stone-950/50 relative p-6">
            <a href={`/api/assets/plans/Foyer_${activeFloor}.svg`} target="_blank" rel="noopener noreferrer" className="absolute inset-0 p-4">
              <img src={`/api/assets/plans/Foyer_${activeFloor}.svg`} alt="Foyer Plan" className="w-full h-full object-contain brightness-90 saturate-[0.8] contrast-125 hover:brightness-110 transition-all dark:invert" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
            </a>
          </div>
        </div>
      </div>
