        ...
        <div className="space-y-8 pb-20">
            ...
            <header className="flex items-end justify-between border-b border-zinc-800/60 pb-6 relative">
                ...
                <div className="hidden md:flex items-center gap-4">
                    <Link href="/admin/maps">
                        <Button variant="outline" className="bg-orange-500/5 border-orange-500/30 text-orange-400 hover:bg-orange-500/10 hover:border-orange-500 hover:text-orange-300 font-mono text-[10px] uppercase tracking-widest h-10 px-6 group rounded-none">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                <div className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/60 p-6 relative overflow-hidden group rounded-none">
                    ...
                    <Users className="w-6 h-6 text-blue-500 mb-4 opacity-50 drop-shadow-[0_0_8px_rgba(59,130,246,0.8)]" />
                ...
                <div className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/60 p-6 relative overflow-hidden group rounded-none">
                    ...
                    <Database className="w-6 h-6 text-emerald-500 mb-4 opacity-50 drop-shadow-[0_0_8px_rgba(16,185,129,0.8)]" />
                ...
                <div className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/60 p-6 relative overflow-hidden group rounded-none">
                    <div className="absolute top-0 left-0 w-1 h-8 bg-orange-500/50" />
                    <Map className="w-6 h-6 text-orange-500 mb-4 opacity-50 drop-shadow-[0_0_8px_rgba(249,115,22,0.8)]" />
                ...
                <div className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/60 p-6 relative overflow-hidden group rounded-none">
                    <div className="absolute top-0 left-0 w-1 h-8 bg-purple-500/50" />
                    <Calendar className="w-6 h-6 text-purple-500 mb-4 opacity-50 drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]" />
                ...
                <div className="bg-zinc-900/40 backdrop-blur-xl border border-red-900/40 p-6 relative overflow-hidden group rounded-none">
                    <div className="absolute top-0 left-0 w-1 h-8 bg-red-500" />
                    <Shield className="w-6 h-6 text-red-500 mb-4 opacity-50 drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
                    <p className="text-[10px] font-mono tracking-widest text-zinc-500 uppercase">Privileged</p>
                    <p className="text-3xl font-mono font-bold text-red-400 mt-1">{telemetry?.counts.users || "---"}</p>
                </div>
            </div>
            ...
            <div className="p-8 bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/60 rounded-none shadow-xl">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-mono text-white uppercase tracking-widest flex items-center gap-3">
                        <Tags className="w-5 h-5 text-blue-500" /> Relationship Label Configuration
                    </h3>
                </div>
                <form onSubmit={handleCreateRelType} className="flex gap-3 mb-6">
                    ...
                    <input type="text" value={relName} onChange={e=>setRelName(e.target.value)} placeholder="New relationship label name..." className="flex-1 bg-black/50 border border-zinc-800 rounded-none px-4 font-mono text-xs uppercase text-white outline-none focus:border-blue-500 transition-all" />
                    <Button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white font-mono uppercase tracking-[0.2em] text-[10px] px-8 rounded-none h-12 border border-blue-400/30 shadow-none">Add Type</Button>
                </form>
                <div className="flex flex-wrap gap-2">
                    {relTypes.map(rt => (
                        <div key={rt.id} className="flex items-center gap-3 bg-zinc-950/80 border border-zinc-800 px-4 py-2 group rounded-none">
                            ...
                            <button onClick={() => deleteRelType(rt.id)} className="text-zinc-700 hover:text-red-500 ml-2 transition-colors"><Trash2 className="w-3.5 h-3.5"/></button>
                        </div>
                    ))}
                </div>
            </div>
                ...
                <div className="p-6 border-b border-zinc-800/60 flex items-center justify-between bg-black/30 backdrop-blur-2xl">
                    <div className="w-[450px]">
                        <SearchBar
                            ...
                            accentColorClass="group-focus-within:bg-red-500"
                        />
                    </div>
                    <div className="flex items-center gap-4 text-zinc-500 font-mono text-[9px] uppercase tracking-widest">
                        <div className="flex items-center gap-2"><div className="w-2 h-2 bg-orange-500/50" /> Residential</div>
                        <div className="flex items-center gap-2"><div className="w-2 h-2 bg-blue-500/50" /> Identity</div>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[1200px]">
                        <thead className="sticky top-0 z-20 bg-zinc-950/95 backdrop-blur-xl border-b border-zinc-800">
                            <tr>
                                ...
                                <th className="py-4 px-6 text-[10px] font-black font-mono text-orange-500 uppercase tracking-[0.2em] w-32">Apartment</th>
                                <th className="py-4 px-6 text-[10px] font-black font-mono text-blue-500 uppercase tracking-[0.2em] w-48 text-center">Access Level</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={s.id} className="hover:bg-zinc-800/30 transition-all group">
                                    ...
                                    <td className="py-3 px-6">
                                        <EditableCell 
                                            ...
                                            className="text-orange-400 font-black focus:bg-orange-500/10 focus:border-orange-500/50" 
                                        />
                                    </td>
                                    <td className="py-3 px-6 text-center">
                                        {s.user_id ? (
                                            <div className="flex items-center justify-center gap-2">
                                                <button onClick={() => toggleAdmin(s)} className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest border transition-all rounded-none ${s.is_admin ? 'bg-red-500/10 border-red-500/40 text-red-400 shadow-[0_0_15px_rgba(239,68,68,0.15)]' : 'bg-zinc-800/50 border-zinc-700/50 text-zinc-500 hover:text-white'}`}>{s.is_admin ? 'L4 Admin' : 'Standard'}</button>
                                                <button onClick={() => revokeUser(s)} className="opacity-0 group-hover:opacity-100 text-red-500/40 hover:text-red-500 transition-all p-1"><X className="w-3.5 h-3.5" /></button>
                                            </div>
                                        ) : (
                                            <button onClick={() => { setProvTarget(s); setProvIsAdmin(false); setProvCredentials(null); }} className="opacity-0 group-hover:opacity-100 text-[9px] font-black uppercase tracking-[0.2em] text-blue-400 border border-blue-500/30 hover:bg-blue-500/10 px-4 py-1.5 rounded-none transition-all">Provision</button>
                                        )}
                                    </td>
                                </tr>
                            ))}
            ...
            {provTarget && (
                <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
                    <div className="bg-zinc-950 border border-blue-500/20 rounded-none p-10 max-w-md w-full shadow-[0_0_100px_rgba(0,0,0,1)] relative overflow-hidden border-t-blue-500">
                        ...
                        <div className="flex justify-between items-start mb-8">
                            <div>
                                <h2 className="text-2xl font-black text-white tracking-tighter uppercase flex items-center gap-4 font-mono">
                                    <Fingerprint className="w-6 h-6 text-blue-500" /> Account Provisioning
                                </h2>
                        ...
                        {!provCredentials ? (
                            <div className="space-y-8 relative z-10">
                                <div className="flex items-center gap-4 py-6 border-y border-zinc-900">
                                    <input type="checkbox" id="modal_is_admin" checked={provIsAdmin} onChange={(e) => setProvIsAdmin(e.target.checked)} className="w-5 h-5 bg-zinc-950 border-zinc-800 text-red-600 focus:ring-0 rounded-none cursor-pointer" />
                                    <label htmlFor="modal_is_admin" className="text-[10px] tracking-[0.2em] font-mono font-black text-zinc-500 select-none flex items-center gap-3 cursor-pointer hover:text-red-400 transition-colors uppercase"><Shield className="w-4 h-4 text-red-500/50" /> Administrator Privileges</label>
                                </div>
                                <Button onClick={handleProvisionUser} disabled={provLoading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-mono uppercase tracking-[0.3em] font-black h-14 rounded-none border border-blue-400/30 shadow-none transition-all">{provLoading ? "Synchronizing..." : "Generate Access Key"}</Button>
                            </div>
                        ) : (
                            <div className="space-y-6">
                                <div className="p-6 bg-emerald-950/10 border border-emerald-500/20 rounded-none text-center">
                                    <p className="text-[9px] font-mono text-emerald-500/70 uppercase tracking-widest mb-4">Secure Identity Generated</p>
                                    <div className="space-y-2">
                                        ...
                                        <div className="bg-black/60 p-4 border border-zinc-900 font-mono rounded-none">
                                            ...
                                            <p className="text-emerald-400 text-lg font-black tracking-widest">{provCredentials.password}</p>
                                        </div>
                                    </div>
                                </div>
...
function EditableCell({ value, onSave, className = "" }: { value: string, onSave: (v: string) => void, className?: string }) {
    ...
    return (
        <input 
            ...
            className={`w-full bg-transparent border border-transparent hover:border-zinc-800 focus:border-red-500/30 focus:bg-zinc-950 px-2 py-1 outline-none transition-all text-zinc-300 placeholder-zinc-800 font-mono rounded-none ${className}`}
            placeholder="..."
        />
    );
}
