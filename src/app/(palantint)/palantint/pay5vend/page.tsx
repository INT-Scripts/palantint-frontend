...
import { Download, Coffee, ShieldAlert, Cpu } from "lucide-react";
        ...
        const progressInterval = setInterval(() => {
          setDownloadProgress(prev => {
            if (prev >= 100) {
              ...
              setTimeout(async () => {
                try {
                  const response = await fetch(`${API_BASE_URL}/private/pay5vend/download`, { headers: getAuthHeaders() });
                  if (!response.ok) {
            ...
            <Card className="border-zinc-800 bg-zinc-950/40 backdrop-blur-xl rounded-none relative overflow-hidden">
              ...
              <CardContent className="p-8 space-y-6">
                ...
                <div className="grid grid-cols-1 gap-6 font-mono uppercase">
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] text-zinc-500 block">Signal Strength (RSSI)</span>
                    <span className="text-2xl sm:text-3xl font-black text-rose-400">{rssi} dBm</span>
                  </div>
                </div>
              </CardContent>
            </Card>
