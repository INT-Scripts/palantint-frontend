...
export default function StudentProfilePage() {
    ...
    useEffect(() => {
        fetchPrivate(`/students/${id}`)
            .then(data => {
                setStudent(data);
                if (data && data.first_name) {
                    document.title = `${data.first_name} ${data.last_name}`;
                    fetchPrivate(`/students/${id}/recently-viewed`, { method: "POST" }).catch(() => { });
                }
            })
            .catch(err => {
    ...
    const saveEdit = async () => {
        setEditLoading(true);
        try {
            const updated = await fetchPrivate(`/students/${student.id}`, {
                method: "PATCH",
    ...
    const imgUrl = getStudentImageUrl(student.id);
                                ...
                                <>
                                    <div className="text-center sm:text-left flex-1 space-y-4">
                                        ...
                                        <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-6 mt-4 pt-4 border-t border-zinc-800/60">
                                            ...
                                            {student.apartment && (
                                                <button
                                                    onClick={() => router.push(`/palantint/apartments?room=${student.apartment}`)}
                                ...
                                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 flex flex-col">
                                    ...
                                    <div className="flex-1 p-6 relative">
                                        ...
                                        <TabsContent value="media" className="m-0 h-full">
                                            <MediaGallery studentId={student.id} initialMedia={student.media} themeColor={PALETTE.media[500]} />
                                        </TabsContent>
