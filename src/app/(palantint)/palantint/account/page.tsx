...
import { 
    ...
    BookOpen, X, ArrowRight, User as UserIcon
} from "lucide-react";
...
export default function AccountPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                try {
                    const me = await fetchPrivate("/users/me/student");
                    setStudent(me);
                    fetchPrivate(`/students/${me.id}/agenda`).then(setEvents);
                    
                    if (me.apartment) {
                        fetchPrivate("/students/apartments/occupied").then(allOccupiedDict => {
    ...
    const handleSaveCas = async (e: React.FormEvent) => {
        ...
        setCasSaving(true);
        try {
            await fetchPrivate("/users/me/cas-credentials", {
                method: "POST",
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-zinc-100 selection:bg-student-500/30 font-sans">
                            ...
                            <div className="p-8">
                                <div className="flex flex-col items-center text-center">
                                    <div className="relative mb-8 group/avatar">
                                        ...
                                        <div className="relative w-40 h-40 border border-zinc-800 p-1 bg-zinc-950/50 rounded-none">
                                            <Avatar className="h-full w-full rounded-none">
                                                <AvatarImage 
                                                    src={student ? getStudentImageUrl(student.id) : undefined} 
                                    ...
                                    <div className="w-full mt-10 grid grid-cols-1 gap-3">
                                        <Link href={student ? `/palantint/students/${student.id}` : "#"} className="block w-full">
                                        ...
                                        {user?.is_admin && (
                                            <Link href="/palantint/admin" id="admin-console-link" className="block w-full">
                    ...
                    <div className="lg:col-span-1 space-y-8">
                        ...
                        <section className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/60 p-6 rounded-none shadow-xl">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {neighbors.length > 0 ? (
                                    neighbors.map((n: any) => (
                                        <Link key={n.id} href={`/palantint/students/${n.id}`} className="group/n">
                                            <div className="relative border border-zinc-800 bg-black/40 p-3 hover:border-housing-500/30 text-center rounded-none">
                                                <Avatar className="h-10 w-10 mx-auto mb-2 rounded-none grayscale group-hover/n:grayscale-0">
                                                    <AvatarImage src={getStudentImageUrl(n.id)} />
            ...
            {selectedEventId && (
                <div 
                    className="fixed right-8 top-24 z-50 w-80 bg-zinc-900/90 backdrop-blur-2xl border border-zinc-800 p-0 shadow-[0_0_40px_rgba(0,0,0,0.8)] animate-in slide-in-from-right duration-300 overflow-hidden"
